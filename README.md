# ✋ Gesture-Driven Particle Universe

An interactive **3D particle system controlled entirely by hand gestures** using  
**MediaPipe Hands, Three.js, Fingerpose and real-time physics** — no controllers, no keyboard, just your hands.

This project turns webcam hand-tracking into a **gesture-controlled creative engine & mini arcade game**.

---

## 🚀 Features

| Gesture / Action | Effect |
|-----------------|--------|
| 🤏 Pinch | Scale the entire particle galaxy |
| 👍 Thumbs-Up | Particles are pulled toward your hand (gravity well) |
| ✌ Peace | Particles explode away from your hand (repulsion field) |
| 👉 Swipe Left / Right | Switch between particle presets |
| ✍ Draw in Air | Paint glowing cosmic trails |
| 🎯 Touch Floating Bubbles | Pop bubbles & increase score |
| 🖐 Fast Motion | Creates brighter, faster particle storms |

---

## 🧠 Tech Stack

- **Three.js** – GPU-accelerated 3D rendering  
- **MediaPipe Hands** – real-time hand landmark tracking  
- **Fingerpose** – gesture recognition engine  
- **Custom Physics Engine** – attraction & repulsion fields  
- **Pure Browser Runtime** – no backend, no installations

---

## 🎮 Arcade Mode

The project contains a hidden **gesture-controlled mini-game**:

- Floating bubbles spawn every 2 seconds  
- Touch a bubble with your fingertip to pop it  
- Score updates live on screen  
- Entirely controlled using your hand — no mouse or keyboard

---

## 🛠 How To Run

```bash
git clone https://github.com/barnikbasu/hand-gesture-particles

Open index.html using GitHub Pages or any static server.
Allow webcam permission and start interacting instantly.

## 💡 Why This Project Matters

This is not just a visual demo — it is a human-computer interaction experiment that demonstrates:

Vision-based interaction replacing hardware controllers

Gestures as a full interaction language

Generative particles behaving like living matter through physics fields

It showcases skills in:
## Computer Vision · Real-Time Rendering · Gesture UX · Creative Coding

## 📈 Future Enhancements

Multiplayer shared canvas

WebXR / AR integration

Neural-network based gesture classifier

Sound-reactive particle fields

👤 Author

Barnik Basu
Creative Technologist | Computer Vision | Interactive Media

If this project impressed you, feel free to ⭐ the repository.
